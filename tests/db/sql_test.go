...
func TestMySQL(t *testing.T) {
	...
	p := peer.NewGenericPeer("mysql.Connector", "hotel", "dbmanger:edJelrZuAgpNeLyn@(localhost:3306)/mysql", nil)
	...
	op.Operate(func(rawClient interface{}) interface{} {
		...
		// 查找默认用户
		mysql.NewWrapper(client).Query("select User from userinfo").Each(func(wrapper *mysql.Wrapper) bool {
